...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *customBackButton = [[UIBarButtonItem alloc] initWithTitle:LocalString(@"UGCKit.Common.Back")
                                                                         style:UIBarButtonItemStylePlain
                                                                        target:self
                                                                        action:@selector(goBack)];
    self.navigationItem.leftBarButtonItem = customBackButton;
    customBackButton.tintColor = UIColorFromRGB(0x0accac);
    self.navigationItem.title = LocalString(@"TCVideoEditPrevView.TitleVideoPreview");
...
- (UIView*)generatingView
{
    ...
    if (!_generationView) {
        ...
        _generationTitleLabel.text = LocalString(@"TCVideoEditPrevView.VideoSynthesizing");
...
- (void)onVideoEnterBackground
{
    ...
    if (_generationView && !_generationView.hidden) {
        ...
        [self _alertWithTitle:LocalString(@"TCVideoEditPrevView.HintVideoSynthesizeFailed")
                      message:LocalString(@"TCVideoEditPrevView.ErrorSwitchBackend")];
    }
}
...
-(void) onJoinComplete:(TXJoinerResult *)result
{
    ...
    if(result.retCode == 0)
    {
        ...
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        NSString *message = [NSString stringWithFormat:LocalString(@"UGCKit.Common.HintErrorCodeMessage"),(long)result.retCode,result.descMsg];
        [self _alertWithTitle:LocalString(@"TCVideoEditPrevView.HintVideoSynthesizeFailed")
                      message:message];
    }
}
...
- (void)_alertWithTitle:(NSString *)title message:(NSString *)message {
    ...
    [controller addAction:[UIAlertAction actionWithTitle:LocalString(@"UGCKit.Common.GotIt")
