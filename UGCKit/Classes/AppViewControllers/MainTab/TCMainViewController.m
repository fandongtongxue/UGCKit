...
@interface TCMainViewController ()<UITabBarControllerDelegate, UIGestureRecognizerDelegate>
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (hasDraft) {
        UIAlertController *controller = [UIAlertController alertControllerWithTitle:LocalString(@"Common.Hint")
                                                                            message:LocalString(@"TCVideoRecordView.ResumeRecord")
                                                                     preferredStyle:UIAlertControllerStyleAlert];
        [controller addAction:[UIAlertAction actionWithTitle:LocalString(@"Common.OK")
...
- (void)setupViewControllers {
    WEAKIFY(self);
    UIViewController *showVC = [UIViewController new];
    UIViewController *_ = [UIViewController new];
    UIViewController *accountInfoViewController = [[UIViewController alloc] init];
    self.viewControllers = @[showVC, _, accountInfoViewController];
    
    [self addChildViewController:showVC imageName:@"video_normal" selectedImageName:@"video_click" title:nil];
...
- (void) initBottomView
{
    UIImage *shadowImage = [[UGCKitTheme sharedTheme] imageNamed:@"tabBarShadow"];
    UIImage *shadowLine = [[UGCKitTheme sharedTheme] imageNamed:@"tabBarShadow_line"];

    CGFloat lineWidth = ([UIScreen mainScreen].bounds.size.width-shadowImage.size.width)/2.0;

    UIGraphicsBeginImageContextWithOptions(CGSizeMake([UIScreen mainScreen].bounds.size.width, shadowImage.size.height),NO,0);
    ...
    imageHidden.image = [[UGCKitTheme sharedTheme] imageNamed:@"hidden"];
    ...
    UIButton *(^createButton)(NSString *title, NSString *imageName, SEL action) = ^(NSString *title, NSString *imageName, SEL action) {
        ...
        [button setImage:[[UGCKitTheme sharedTheme] imageNamed:imageName] forState:UIControlStateNormal];
        [button addTarget:self action:action forControlEvents:UIControlEventTouchUpInside];
        button.titleLabel.font = [UIFont systemFontOfSize:12];
        [button setTitle:LocalString(title) forState:UIControlStateNormal];
        [button sizeToFit];
        return button;
    };
...
- (void)addChildViewMiddleBtn {
    if (nil == self.liveBtn) {
        self.liveBtn = ({
            ...
            [btn setImage:[[UGCKitTheme sharedTheme] imageNamed:@"play_normal"] forState:UIControlStateNormal];
            [btn setImage:[[UGCKitTheme sharedTheme] imageNamed:@"play_click"] forState:UIControlStateSelected];
...
- (void)addChildViewController:(UIViewController *)childController imageName:(NSString *)normalImg selectedImageName:(NSString *)selectImg title:(NSString *)title {
    ...
    if (normalImg.length > 0) {
        childController.tabBarItem.image = [[[UGCKitTheme sharedTheme] imageNamed:normalImg] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    }
    if (selectImg.length > 0) {
        childController.tabBarItem.selectedImage = [[[UGCKitTheme sharedTheme] imageNamed:selectImg] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    }
...
-(void)onVideoBtnClicked
{
    ...
    watermark.image = [[UGCKitTheme sharedTheme] imageNamed:@"watermark"];
...
-(void)onVideoChorusSelectClicked
{
    ...
    _hub.label.text = LocalString(@"TCVodPlay.VideoLoading");
...
-(void)onloadVideoProcess:(CGFloat)process {
    _hub.label.text = [NSString stringWithFormat:LocalString(@"TCVodPlay.VideoLoadingFmt"),(int)(process * 100)];
}
...
-(void)onloadVideoComplete:(NSArray<NSString *> *)videoPaths recordStyle:(UGCKitRecordStyle)recordStyle {
    if (videoPaths.count) {
        ...
        _hub.label.text = LocalString(@"TCVodPlay.VideoLoadFailed");
