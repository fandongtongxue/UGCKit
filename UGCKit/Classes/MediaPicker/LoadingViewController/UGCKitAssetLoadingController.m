...
//        self.loadingProgressView.image = [[UGCKitTheme sharedTheme] imageNamed:[NSString stringWithFormat:@"video_record_share_loading_%d", (int)(total * 8)]];
    });
}
...
//        self.loadingProgressView.image = [[UGCKitTheme sharedTheme] imageNamed:[NSString stringWithFormat:@"video_record_share_loading_%d", 0]];
    } else {
        if (self.completion) {
            UGCKitResult *result = [[UGCKitResult alloc] init];
            result.code = ret;
